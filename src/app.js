...
function extraerYoutubeId(referencia = '') {
  ...
  try {
    ...
    if (host.endsWith('youtube.com')) {
      ...
      const marcador = partes.findIndex((parte) => ['live', 'embed', 'shorts'].includes(parte));
...
function actualizarInformacion() {
  ...
  ui.status.textContent = configuracion?.transmision?.leyendaEstado ||
    (configuracion?.transmision?.enVivo ? 'EN VIVO' : 'TRANSMISIÓN CONTINUA');
  ui.description.textContent = configuracion?.transmision?.descripcion ||
